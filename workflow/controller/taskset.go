...
func (woc *wfOperationCtx) patchTaskSet(ctx context.Context, patch interface{}, pathTypeType types.PatchType) error {
	...
	_, err = woc.controller.wfclientsets[common.LocalCluster].ArgoprojV1alpha1().WorkflowTaskSets(woc.wf.Namespace).Patch(ctx, woc.wf.Name, pathTypeType, patchByte, metav1.PatchOptions{})
	if err != nil {
...
func (woc *wfOperationCtx) createTaskSet(ctx context.Context) error {
	...
	_, err := woc.controller.wfclientsets[common.LocalCluster].ArgoprojV1alpha1().WorkflowTaskSets(woc.wf.Namespace).Create(ctx, &taskSet, metav1.CreateOptions{})
